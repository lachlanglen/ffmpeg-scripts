...
const createLayoutB = async () => {
  try {
    ...
    const scaledHeight = Math.floor(scaledWidth / 9 * 16);
    const totalHeight = scaledHeight + croppedHeight;
    ...
    const combinedArgs = [
      ...
      '-i', overlay,
      '-filter_complex', `[0:v]crop=${croppedWidth}:${croppedHeight}:0:${offset},fps=30[v0];${!areSameWidth ? `[${input1.isWidest ? 'v0' : '1:v'}]scale=${scaledWidth}:${scaledHeight},fps=30[${input1.isWidest ? 'v0s' : 'v1'}];` : '[1:v]fps=30[v1];'}[${vstackInput1}][${vstackInput2}]vstack=inputs=2,split=3[lc][m][rc];[lc]crop=${singleBlurWidth}:ih:0:0,boxblur=luma_radius=${Math.floor(singleBlurWidth / 2)}:chroma_radius=10:luma_power=1[blurredA];[rc]crop=${singleBlurWidth}:ih:${scaledWidth - singleBlurWidth}:0,boxblur=luma_radius=${Math.floor(singleBlurWidth / 2)}:chroma_radius=10:luma_power=1[blurredB];[blurredA][m][blurredB]hstack=inputs=3[combined];[combined][2]overlay=10:main_h-overlay_h-10[v];[0:a][1:a]amix[a]`,
      '-map', '[v]', // map video
      '-map', '[a]', // map audio
      stackedOutput,
    ];
